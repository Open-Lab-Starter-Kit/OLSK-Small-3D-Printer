import { useBounds, Clone, Outlines, Edges } from "@react-three/drei"
...
export default function Model({ modelIn, modelOut, modelInCopy, modelInCopy2, modelOutCopy }) {
    ...
    const preparingStepArray = [ //shown grouped
        [
            "031_Prepare_the_bed_1",
            "032_Prepare_the_bed_2",
            "033_Prepare_the_bed_3"
        ],
        [
            "05_Prepare_back_panels"
        ],
        [
            "081_Prepare_the_gantry_frame_1",
            "082_Prepare_the_gantry_frame_2",
            "083_Prepare_the_gantry_frame_3",
            "084_Prepare_the_gantry_frame_4",
            "085_Prepare_the_gantry_frame_5"
        ],
        [
            "09_Prepare_bottom_panel"
        ],
        [
            "11_Prepare_top_panel"
        ],
        [
            "151_Prepare_the_head_1",
            "152_Prepare_the_head_2",
            "16_Prepare_the_X_axis"

        ],
        [
            "201_Prepare_electronic_panel_1",
            "202_Prepare_electronic_panel_2",
            "203_Prepare_electronic_panel_3"
        ],
        [
            "27_Prepare_the_display"
        ],
        [
            "291_Prepare_the_side_panel_1",
            "292_Prepare_the_side_panel_2"
        ],
        [
            "34_Prepare_back_lid"
        ],
        [
            "381_Prepare_the_doors_1",
            "382_Prepare_the_doors_2"
        ]
    ]
    ...
    const mainMachineBuildStepChange = useCallback(() => {
        ...
        let geometriesArray = []
        let excludeArray = []
        if (stepName[stepCount] === "16_Prepare_the_X_axis") {
            excludeArray = [
                "152_Prepare_the_head_2"
            ]
            let exclude = modelAux.getObjectByName("25_Prepare_Z_ball_screw", true)
            console.log("exception")
        }
        //Go through each item of the mainMachineBuildArray and display only what is not in the exception list
        mainMachineBuildArray.filter(obj => !exceptionArray.some(arr => arr.includes(obj.name)) && !exceptionArray.some(arr => arr.includes(obj.name)))
            .forEach(obj => {
                obj.visible = true;
                let clonedObj = obj.clone();
